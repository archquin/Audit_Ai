...
for i in range(len(semantics)):
    semantics[i] = sorted(set(semantics[i]))    
...
for i in range(len(sembling)):
    if sembling[i] == suun[Xx] and Xx < len(suun)-1:
        yt.append(i)
        Xx+=1
...
###In other words, between non-trivial occurences all possible combinations of the entity's elements must be semantically entailed!
...
zlast.append([suun[16]])
SS.append([zlast[16][0][0]])

GG.append([zlast[16][0][1]])

FF.append([zlast[16][0][2]])
            ...
            if j == SS[i-1][kij]:
                ...
                for kji in iji:
                    ...
                    if val == True:
                        fil = kji
                        if ijk[0:5] != 'PartS' :
                            Dic.update({j:{kji:{ijk:kij}}})
                            Xan.update({ijk:Dic})
                        elif ijk[0:5] == 'PartS' :
                            cfd = list(Xan.keys())
                            for rof in cfd:  
                                Dic.update({kji:{ijk:Xan[rof][ijk]}})
                                Xan[rof].update({ijj:Dic})
                        Unsemble.update(Xan)
                    elif val == False:
                        fil = jik
                        if ijk[0:5] != 'PartS' :
                            Dic3.update({j:{kji:{ijk:kij}}})
                            Dic2.update({j:{jik:{ijk:kij}}})
                            Dic4 = {j:{kji:{ijk:Dic3[j][kji][ijk]}}}
                            Dic4[j][kji].update(Dic2[j][jik])
                            Xan2.update({ijk:Dic4})
                        elif ijk[0:5] == 'PartS' :
                            cfd = list(Xan2.keys())
                            for rof in cfd:  
                                Dic3.update({kji:{ijk:Xan2[rof][ijk]}})
                                Dic2.update({jik:{ijk:Xan2[rof][ijk]}})
                                Dic4 = {kji:{ijk:Dic3[kji][ijk]}}
                                Dic4.update(Dic2[jik][ijk])
                                Xan2[rof].update({ijj:Dic4})
