...
######  An introdoction to python data abstract anlysis through symbols that can be interpreted to serve sound as an AudItAI in an Database  #####
...
########################################## Implementation of monitoring resolution be tested objectively #######################################

# Although even the most intricate in nature assembly is a fairly easy distilled to each components, that are accompanied by set of

# Instructions, monitoring was a matter of practicality of implementation. 
#############################################################################################################################################
...
for l in range(len(semantics)):
    ...
    if len(semantics3[l]) > 1:
        for n in range(1,len(semantics3[l])):
            qn = 0
            for p in semantics[l][n]:
                if qn == 0:
                    w2 = trnslate(p)
                    r = [p,URdmb[semantic[l]],semantic[l],0] 
                    suun2.append(r)
                qn += 1
...
for f in range(len(suuun)):
    suuuun  = []
    if suuun[f] != suuun1[f]:
        for c in range(1,len(semantics3[f])):
...
for i in range(len(zlast)):
    if len(semantics3[i]) == 1:
        ...
        BB = []
        CC = []
        for k in range(len(semantics3[i])):
            A = []
            B = []
            C = []
            for j in zlast[i][k]:
...
for i in range (1,len(SS)+1):
    Xan = {}
    Xut = {}
    for ij in range(len(semantics3[i-1])):
        if ij == 0:
            for j in semantics2[i-1]: 
                Dic = {}
                k = len(SS[i-1])
                for kij in range(0,k):
                    if j == SS[i-1][kij]:
            ...
            Xil = {}
            for ik in range(0,ij+1):
                for j in semantics[i-1][ik]: 
                    Dic = {}
                    k = len(SS[i-1][ik])
                    nfc = semantics3[i-1][ik]
                    for kij in range(0,k):
                        if j == SS[i-1][ik][kij]:
                            jik = GG[i-1][ik][kij]
                            ijk = FF[i-1][ik][kij]
                            ...
                            if ijk[0:5] != 'PartS' :
                                ...
                                if nfc != ijj :
                                    dfc = semantic[i-1]
                                    Dic.update({jik:{ijk:Xil[dfc][ijk]}})
                                    Xil[dfc].update({ijj:Dic})
                                    Xan.update(Xil[dfc])
                                elif nfc == ijj and ik > 0:
                                    
                                    Xan.update(Xil[dfc])
                                    Xan[nfc].update({jik:Xil[dfc]})
                                    print('Here',Xan)
                                    
                                    
                    Unsemble.update({semantic[i-1]:Xan})
...
##########################################     A Meta-Analysis of an unsuccesful sound approach      #############################################
...
#### P.M2.! I don t know how many more trivialty there one can find on the Unsemble dictionary creation, yet rest assured

# it might not have been tested enough.
